    ...
    ERC20Upgradeable
{
    ...
    /*----------------- events -----------------*/
    event TreasuryLockTransfer(address indexed validator, uint256 amount);
    ...
    function _bootstrapInitialHolder(
        ...
        // transfer 1 DC to Treasury (native DC)
        address lockAmountTreasuryAddress = IStakeHub(STAKE_HUB_ADDR)
            .lockAmountTreasuryAddress();
        (bool success, ) = lockAmountTreasuryAddress.call{value: toLock}("");
        if (!success) revert TransferFailed();
        emit TreasuryLockTransfer(validator, toLock);

        // mint initial tokens to the validator (50 shares)
        uint256 initShares = initAmount - toLock;
        _mint(validator, initShares);

        totalPooledBNB = initShares;
    }
