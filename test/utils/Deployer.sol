...
import "./interface/IDorsenValidatorSet.sol";
...
import "./interface/IDorsenGovernor.sol";
import "./interface/IGovToken.sol";
import "./interface/IDorsenTimelock.sol";
...
contract Deployer is Test {
    ...
    DorsenValidatorSet public dorsenValidatorSet;
    ...
    DorsenGovernor public governor;
    GovToken public govToken;
    DorsenTimelock public timelock;
    ...
    constructor() {
        ...
        dorsenValidatorSet = DorsenValidatorSet(VALIDATOR_CONTRACT_ADDR);
        vm.label(address(dorsenValidatorSet), "Validator");
        ...
        governor = DorsenGovernor(GOVERNOR_ADDR);
        vm.label(address(governor), "DorsenGovernor");
        govToken = GovToken(GOV_TOKEN_ADDR);
        vm.label(address(govToken), "GovToken");
        timelock = DorsenTimelock(TIMELOCK_ADDR);
        vm.label(address(timelock), "DorsenTimelock");
        tokenRecoverPortal = TokenRecoverPortal(TOKEN_RECOVER_PORTAL_ADDR);
        vm.label(address(tokenRecoverPortal), "TokenRecoverPortal");

        // set the latest code
        bytes memory deployedCode = vm.getDeployedCode("DorsenValidatorSet.sol:DorsenValidatorSet");
        ...
        deployedCode = vm.getDeployedCode("DorsenGovernor.sol:DorsenGovernor");
        ...
        deployedCode = vm.getDeployedCode("DorsenTimelock.sol:DorsenTimelock");
