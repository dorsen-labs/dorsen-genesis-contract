...
interface TokenHub {
    ...
    function TRANSFER_IN_FAILURE_INSUFFICIENT_BALANCE()
        external
        view
        returns (uint8);

    function TRANSFER_IN_FAILURE_NON_PAYABLE_RECIPIENT()
        external
        view
        returns (uint8);
    ...
    function lockInfoMap(
        address,
        address
    ) external view returns (uint256 amount, uint256 unlockAt);
